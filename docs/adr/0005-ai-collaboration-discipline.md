...
> Update: ADR-0008 将本节弱化为单人项目的轻量测试纪律。严格 TDD 只作为高风险改动的建议路径;完成声明以测试或可复现实证为准,不要求所有改动证明红绿循环。

当前执行以 ADR-0008 为准:

- 高风险改动建议测试先行,但不把红绿循环作为流程门。
- 同构扩展可以直接实现 + 补合同测试。
- 未知 PowerPoint / OOXML / 第三方库行为可以先探索,再沉淀测试、fixture、issue、PRD 或明确限制。
- 完成声明只看测试或可复现实证是否足够支撑结论。

这与 ADR-0004 的 round-trip 约束不冲突:测试仍然是闭环 spec,只是允许开发者按风险选择最轻路径。
