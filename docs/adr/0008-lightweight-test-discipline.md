...
但 `pptchartengine` 目前主要由一个开发者推进。过细的 TDD 分档、流程证明和文档标注会把单人项目拖成团队合规流程。PowerPoint / OOXML / `python-pptx` 又经常需要先探索对象模型,不适合每个小改动都强制红绿循环。
...
否决。`pptchartengine` 的价值来自 PowerPoint 原生可编辑、round-trip、replace invariant 和 fail-loud;没有证据就不能形成可信内核。
